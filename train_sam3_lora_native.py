...
class COCOSegmentDataset(Dataset):
    ...
    def __init__(
        ...
        enable_prompt_pair_sampling: bool = False,
        method_variant: str = "ft_canon",
    ):
        """
        Args:
        ...
        self.enable_prompt_pair_sampling = enable_prompt_pair_sampling
        ...
        if self.enable_prompt_pair_sampling and self.prompt_manager is None:
            raise ValueError("prompt_manager is required when prompt pair sampling is enabled.")

        if self.method_variant not in {"ft_canon", "ft_norm", "ft_norm_group"}:
            raise ValueError(f"Unsupported method_variant: {self.method_variant}")

        if self.method_variant in {"ft_norm", "ft_norm_group"} and self.prompt_manager is None:
            raise ValueError(
                f"prompt_manager is required when method_variant='{self.method_variant}' "
                "so prompt normalization can be applied."
            )

    def _get_canonical_query_text(self, category_name):
        if self.method_variant in {"ft_norm", "ft_norm_group"} and self.prompt_manager is not None:
    ...
    def _build_query(self, query_text, obj_ids, img_id, orig_h, orig_w, category_id):
    ...
    def __getitem__(self, idx):
        ...
        # For training with prompt pairs, each category produces:
        #   1. canonical prompt (from COCO category name)
        #   2. synonym prompt (from prompts.yaml)
        ...
        if len(class_to_query_info) > 0:
            for query_info in class_to_query_info.values():
                ...
                if self.enable_prompt_pair_sampling:
                    canonical_prompt = self._get_canonical_query_text(category_name)
                    queries.append(
                        self._build_query(
                            canonical_prompt, obj_ids, img_id, orig_h, orig_w, category_id
                        )
                    )

                    if self.prompt_manager.has_synonym(category_name):
                        synonym_prompt = self.prompt_manager.sample_synonym_prompt(
                            category_name, allow_fallback=True
                        )
                        queries.append(
                            self._build_query(
                                synonym_prompt, obj_ids, img_id, orig_h, orig_w, category_id
                            )
                        )
                else:
                    queries.append(
                        self._build_query(
                            self._get_canonical_query_text(category_name),
                            obj_ids,
                            img_id,
                            orig_h,
                            orig_w,
...
class SAM3TrainerNative:
    def __init__(self, config_path, multi_gpu=False, weights_path=None):
        self.config_path = Path(config_path).resolve()
        with open(config_path, "r") as f:
            self.config = yaml.safe_load(f)

        self.training_cfg = self.config.get("training", {})
        self.method_variant = self.training_cfg.get("method_variant", "ft_canon")
        self.lambda_group = float(self.training_cfg.get("lambda_group", 0.0))
        self.lambda_canon = float(self.training_cfg.get("lambda_canon", 0.0))
        self.enable_prompt_pair_sampling = bool(
            self.training_cfg.get("enable_prompt_pair_sampling", self.method_variant == "ft_norm_group")
        )

        if self.method_variant not in {"ft_canon", "ft_norm", "ft_norm_group"}:
            raise ValueError(f"Unsupported training.method_variant: {self.method_variant}")

        if self.method_variant == "ft_norm_group" and not self.enable_prompt_pair_sampling:
            print_rank0(
                "[Training] enable_prompt_pair_sampling was disabled but method_variant=ft_norm_group "
                "requires canonical+synonym pairs. Forcing enable_prompt_pair_sampling=True."
            )
            self.enable_prompt_pair_sampling = True
        if self.method_variant != "ft_norm_group" and self.enable_prompt_pair_sampling:
            print_rank0(
                f"[Training] enable_prompt_pair_sampling is ignored for method_variant={self.method_variant}. "
                "Using canonical-only training queries."
            )
            self.enable_prompt_pair_sampling = False
        if self.method_variant != "ft_norm_group" and self.lambda_group != 0.0:
            print_rank0(
                f"[Training] lambda_group={self.lambda_group:.4f} configured for method_variant={self.method_variant}, "
                "but group consistency loss is only active for ft_norm_group."
            )

        self.prompts_cfg = self.config.get("prompts", {})
        self.train_prompt_manager = None
        self._warned_group_pairing_issue = False
        self.category_id_to_name = {}

        prompt_features_required = self.method_variant in {"ft_norm", "ft_norm_group"}
        if self.prompts_cfg.get("enabled", False) or prompt_features_required:
            prompt_config_path = resolve_config_relative_path(
                self.config_path, self.prompts_cfg.get("config_path")
            )
            if prompt_config_path is None or not prompt_config_path.exists():
                raise FileNotFoundError(
                    f"Prompt config file not found: {self.prompts_cfg.get('config_path')}"
                )

            dataset_key = self.prompts_cfg.get("dataset_key")
            if not dataset_key:
                raise ValueError("prompts.dataset_key must be set when prompts are enabled.")

            self.train_prompt_manager = PromptManager(
                prompt_config_path=prompt_config_path,
                dataset_key=dataset_key,
                warning_fn=print_rank0,
            )
        elif prompt_features_required:
            raise ValueError(
                f"prompts.enabled must be true for method_variant='{self.method_variant}'"
            )
        ...
        if checkpoint_path:
            print_rank0(f"Using local SAM3 checkpoint: {checkpoint_path}")
        elif not load_from_hf:
            raise ValueError(
                "No SAM3 base checkpoint configured. Set model.checkpoint_path to a local "
                "sam3.pt file or enable model.load_from_hf."
            )
        ...
        # Optionally initialize LoRA adapters from an existing checkpoint.
        pretrained_lora = weights_path or self.config.get("output", {}).get("pretrained_lora_weights")
        if pretrained_lora:
            print_rank0(f"Loading initial LoRA weights from: {pretrained_lora}")
            load_lora_weights(self.model, pretrained_lora)
    ...
    def _warn_group_pairing_issue(self, message):
        if self._warned_group_pairing_issue:
            return
        self._warned_group_pairing_issue = True
        print_rank0(f"[Group Loss Warning] {message}")

    def _compute_group_consistency_loss(self, input_batch, final_outputs):
        if self.method_variant != "ft_norm_group":
            return final_outputs["pred_masks"].new_zeros(())
        ...
        category_ids = stage_metadata.original_category_id.detach().cpu().tolist()

        grouped_indices = defaultdict(lambda: {"canonical": [], "synonym": []})
        for query_idx, (text_id, local_img_id, category_id) in enumerate(
            zip(text_ids, local_img_ids, category_ids)
        ):
            query_text = input_batch.find_text_batch[text_id]
            category_name = self.category_id_to_name.get(int(category_id), "object")
            canonical_prompt = self.train_prompt_manager.get_canonical_prompt(category_name)
            query_role = "canonical" if query_text == canonical_prompt else "synonym"
            group_key = (int(local_img_id), int(category_id), canonical_prompt)
            grouped_indices[group_key][query_role].append(query_idx)

        pair_indices = []
        for group_key, query_group in grouped_indices.items():
            canonical_indices = query_group["canonical"]
            synonym_indices = query_group["synonym"]
            if not canonical_indices or not synonym_indices:
                self._warn_group_pairing_issue(
                    f"Unable to form canonical/synonym pair for group={group_key}. "
                    "Skipping group consistency loss for this group."
                )
                continue
            if len(canonical_indices) != len(synonym_indices):
                self._warn_group_pairing_issue(
                    f"Mismatched pair count for group={group_key}: "
                    f"{len(canonical_indices)} canonical vs {len(synonym_indices)} synonym queries. "
                    "Pairing by query order up to the shorter side."
                )
            pair_count = min(len(canonical_indices), len(synonym_indices))
            for pair_idx in range(pair_count):
                pair_indices.append((canonical_indices[pair_idx], synonym_indices[pair_idx]))
        ...
        for canonical_idx, synonym_idx in pair_indices:
            pair_losses.append(
                torch.mean((pred_masks[canonical_idx] - pred_masks[synonym_idx]) ** 2)
            )

        return torch.stack(pair_losses).mean()

    def _append_jsonl(self, output_path, payload):
    ...
    def train(self):
        ...
        train_ds = COCOSegmentDataset(
            data_dir=data_dir,
            split="train",
            prompt_manager=self.train_prompt_manager,
            enable_prompt_pair_sampling=self.enable_prompt_pair_sampling,
            method_variant=self.method_variant,
        )
        self.category_id_to_name = dict(train_ds.categories)
        ...
        try:
            ...
            val_ds = COCOSegmentDataset(
                data_dir=data_dir,
                split="valid",
                prompt_manager=self.train_prompt_manager if self.method_variant in {"ft_norm", "ft_norm_group"} else None,
                enable_prompt_pair_sampling=self.enable_prompt_pair_sampling,
                method_variant=self.method_variant,
            )
            if len(val_ds) > 0:
        ...
        print_rank0(
            f"Training method_variant={self.method_variant} | "
            f"prompt_pair_sampling={self.enable_prompt_pair_sampling} | "
            f"lambda_group={self.lambda_group:.4f}"
        )
        if self.lambda_canon != 0.0:
            print_rank0(
                f"Training lambda_canon={self.lambda_canon:.4f} (reserved; current loss uses L_seg + lambda_group * L_group)"
            )

        if self.train_prompt_manager is not None:
            if self.enable_prompt_pair_sampling:
                prompt_report = self.train_prompt_manager.warn_for_missing_categories(
                    train_ds.categories.values()
                )
                prompt_examples = self.train_prompt_manager.sample_prompt_examples(
                    train_ds.categories.values(),
                    num_examples=3,
                )
            else:
                prompt_report = self.train_prompt_manager.get_category_report(
                    train_ds.categories.values()
                )
                prompt_examples = []

            print_rank0(
                f"Prompt config loaded: dataset_key={self.prompts_cfg['dataset_key']}, "
                f"config={self.prompts_cfg['config_path']}"
            )
            print_rank0(
                f"Prompt canonical categories: {', '.join(prompt_report['canonical_categories'])}"
            )
            if prompt_examples:
                for canonical_prompt, synonym_prompt in prompt_examples:
                    print_rank0(
                        f"Prompt sample: {canonical_prompt} -> {synonym_prompt}"
                    )
        ...
        train_batch_stats_path = out_dir / "train_batch_stats.jsonl"
        val_batch_stats_path = out_dir / "val_batch_stats.jsonl"
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch_dict in enumerate(pbar):
                ...
                group_loss = self._compute_group_consistency_loss(input_batch, final_outputs)

                # Extract total loss
                seg_loss = loss_dict[CORE_LOSS_KEY]
                total_loss = seg_loss + (self.lambda_group * group_loss)
                ...
                train_group_losses.append(group_loss.item())
                if self.method_variant == "ft_norm_group":
                    self._append_jsonl(
                        train_batch_stats_path,
                        {
                            "epoch": epoch + 1,
                            "batch": batch_idx,
                            "phase": "train",
            ...
            avg_train_seg_loss = sum(train_seg_losses) / len(train_seg_losses) if train_seg_losses else 0.0
            avg_train_group_loss = sum(train_group_losses) / len(train_group_losses) if train_group_losses else 0.0
            ...
            if has_validation and val_loader is not None:
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, batch_dict in enumerate(val_pbar):
                        input_batch = batch_dict["input"]
                        input_batch = move_to_device(input_batch, self.device)
                        ...
                        val_group_losses.append(group_loss.item())
                        if self.method_variant == "ft_norm_group":
                            self._append_jsonl(
                                val_batch_stats_path,
                                {
                                    "epoch": epoch + 1,
                                    "batch": batch_idx,
                                    "phase": "val",
                        ...
                        val_pbar.set_postfix({
                            ...
                            "val_group": group_loss.item(),
                        })

                avg_val_loss = sum(val_losses) / len(val_losses)
                avg_val_seg_loss = sum(val_seg_losses) / len(val_seg_losses) if val_seg_losses else 0.0
                avg_val_group_loss = sum(val_group_losses) / len(val_group_losses) if val_group_losses else 0.0
                ...
                if self.method_variant == "ft_norm_group":
                    print_rank0(
                        f"  Train Avg: seg={avg_train_seg_loss:.6f}, group={avg_train_group_loss:.6f}, total={avg_train_loss:.6f}"
                    )
                    print_rank0(
                        f"  Val Avg:   seg={avg_val_seg_loss:.6f}, group={avg_val_group_loss:.6f}, total={avg_val_loss:.6f}"
                    )
                    ...
                    with open(out_dir / "val_stats.json", "a") as f:
                        ...
                        if self.method_variant == "ft_norm_group":
                            stats_payload.update({
                                ...
                                "train_group_loss": avg_train_group_loss,
                                "train_total_loss": avg_train_loss,
                                "val_seg_loss": avg_val_seg_loss,
                                "val_group_loss": avg_val_group_loss,
                                "val_total_loss": avg_val_loss,
                            })
                        f.write(json.dumps(stats_payload) + "\n")
                ...
                if self.method_variant == "ft_norm_group":
                    print_rank0(
                        f"\nEpoch {epoch+1}/{epochs} - Train Avg: seg={avg_train_seg_loss:.6f}, "
                        f"group={avg_train_group_loss:.6f}, total={avg_train_loss:.6f}"
                    )
...
Examples:
  ...
  Multi-GPU (all 4 GPUs):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to existing LoRA weights to initialize from before training"
    )
    parser.add_argument(
        "--local_rank",
        type=int,
    ...
    if num_devices > 1 and not is_torchrun_subprocess:
        ...
        trainer = SAM3TrainerNative(args.config, multi_gpu=multi_gpu, weights_path=args.weights)
        trainer.train()
